...
export class MiikueChartComponent implements OnInit {
  ...
  private async apiGetTimeseriesData(key: string, startTs: number, endTs: number): Promise<ChartDataPoint[]> {
    ...
    try {
      ...
      const seriesName = this.keyToLabel.get(key) || key;
      ...
      console.error('[MiikueChart] API error for key', key, error);
      return [];
    }
  }

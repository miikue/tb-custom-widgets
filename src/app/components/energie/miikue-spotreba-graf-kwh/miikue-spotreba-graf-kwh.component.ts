...
import {
  SpotrebaGrafKwhChartDataPoint as ChartDataPoint,
  MiikueSpotrebaGrafKwhEngineComponent
} from './engine/miikue-spotreba-graf-kwh-engine.component';
...
@Component({
  selector: 'tb-miikue-spotreba-graf-kwh',
  templateUrl: './miikue-spotreba-graf-kwh.component.html',
  styleUrls: ['./miikue-spotreba-graf-kwh.component.scss'],
  standalone: false
})
export class MiikueSpotrebaGrafKwhComponent implements OnInit, OnDestroy {

  @Input() ctx: WidgetContext;
  @ViewChild(MiikueSpotrebaGrafKwhEngineComponent) chartEngine?: MiikueSpotrebaGrafKwhEngineComponent;

  engineCtx: any = {};
  chartData: ChartDataPoint[] = [];
  selectedTimeWindow: TimeWindow;
  selectedAggregationMode: AggregationMode = 'hour';
  isLoading = false;
  ...
  private async loadChartDataForCurrentWindow(): Promise<void> {
    ...
    const mode = this.resolveEffectiveAggregationMode(windowDurationMs);
    ...
    try {
      ...
      for (const item of keyData) {
        ...
        transformedPoints.push({
          ts: this.normalizeTimestampForMode(ts, mode),
  ...
  private aggregatePointsByDay(points: ChartDataPoint[], seriesName: string, color?: string): ChartDataPoint[] {
    if (!points.length) {
      return [];
    }
    ...
    for (const point of points) {
      const dayStartTs = this.getLocalDayStartTs(point.ts);
      dayBuckets.set(dayStartTs, (dayBuckets.get(dayStartTs) || 0) + point.value);
    }

    const result: ChartDataPoint[] = [];
    for (const [dayStartTs, sumValue] of dayBuckets.entries()) {
      result.push({
        ts: dayStartTs,
        value: sumValue,
        name: seriesName,
  ...
  private getLocalDayStartTs(timestamp: number): number {
  ...
  private normalizeTimestampForMode(timestamp: number, mode: AggregationMode): number {
    ...
    switch (mode) {
      case 'hour': {
        ...
        const startTs = Math.floor((timestamp - bucketMs / 2) / bucketMs) * bucketMs;
        return startTs;
      }
      case 'day':
